...
from FlagDao import flagProvider
import sys
...
def main(sourcePath, config):
    ...
    os.rename(sourcePath, sourcePath+"tmp")
    zip(os.path.join(path, "oversea_file"), os.path.join(path, sourcePath))
...
def deletes(sourcePath, config):
    ...
    (filePath, suffix) = os.path.splitext(sourcePath)
    os.rename(sourcePath, filePath+"_tmp"+suffix)
    zip(os.path.join(path, "oversea_file"), os.path.join(path, sourcePath))
...
if __name__ == '__main__':
    ...
    # config = "{\n      \"AndroidManifest\": \"[\\\"provider-{http://schemas.android.com/apk/res/android}name-com.facebook.FacebookContentProvider\\\"]\",\n    \"values\": \"[\\\"string-name-facebook_app_id\\\", \\\"string-name-fb_login_protocol_scheme\\\", \\\"string-name-facebook_client_token\\\"]\"\n}\n"
    # deletes("/Users/tenk.chen/pakage/AarSetting/tenk/OVERSEA-1.7.9_4.aar", config)

    print("aar", sys.argv[1], sep="\n")
    print("config.txt", sys.argv[2], sep="\n")
    deletes(sys.argv[1], openFile(sys.argv[2]))